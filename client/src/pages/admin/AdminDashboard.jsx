...
function getDashboardCategory() {
  try {
    ...
    const pathCategory = normalizeDashboardCategory(path);
    ...
    const queryCategory =
      normalizeDashboardCategory(params.get("category")) ||
      normalizeDashboardCategory(params.get("shopCategory")) ||
      normalizeDashboardCategory(params.get("serviceType")) ||
      normalizeDashboardCategory(params.get("businessType")) ||
      normalizeDashboardCategory(params.get("adminCategory")) ||
      normalizeDashboardCategory(window.location.search);
    ...
    console.warn("ADMIN DASHBOARD CATEGORY ERROR:", e.message);
...
function getAdminToken() {
  try {
    ...
    console.warn("ADMIN TOKEN READ ERROR:", e.message);
...
function normalizeHiddenDashboardText(value) {
  return String(value || "")
    .replace(/\s+/g, "")
    .replace(/[()\[\]{}"'`~!@#$%^&*_=+\\|;:,./?<>-]/g, "")
    .toLowerCase()
...
const HIDDEN_DASHBOARD_SHOP_KEYS =
  HIDDEN_DASHBOARD_SHOP_NAMES.map((name) =>
    normalizeHiddenDashboardText(name)
  );

const ALLOWED_RECENT_DASHBOARD_SHOP_KEYS =
  ALLOWED_RECENT_DASHBOARD_SHOP_NAMES.map((name) =>
    normalizeHiddenDashboardText(name)
  );

function isHiddenDashboardName(value) {
  const text = normalizeHiddenDashboardText(value);

  if (!text) {
    return false;
  }

  return HIDDEN_DASHBOARD_SHOP_KEYS.some(
    (hiddenName) =>
      text === hiddenName ||
...
function isAllowedRecentDashboardShop(item = {}) {
  return (
    ...
    !isHiddenDashboardName(getDashboardItemText(item))
  );
}
...
function sanitizeRecentDashboardShops(items) {
  ...
  toArray(items).forEach((item) => {
    if (
      ...
      uniqueMap.set(getShopIdentity(item), item);
    }
  });
...
function sanitizeDashboardShops(items) {
  const uniqueMap = new Map();
...
function removeHiddenDashboardShopsFromStorage(category) {
  try {
    if (
      typeof window === "undefined" ||
      !window.localStorage
    ...
    keys.forEach((key) => {
      ...
      const originalItems = collectArrayFromStoredValue(rawValue);

      if (!Array.isArray(originalItems)) {
        return;
      }

      const cleanedItems = originalItems.filter(
        (item) =>
          !isHiddenDashboardShop(item) &&
          !isHiddenDashboardName(getDashboardItemText(item))
      );
      ...
      if (Array.isArray(parsed.shops)) {
        nextValue.shops = cleanedItems;
      } else if (Array.isArray(parsed.items)) {
        nextValue.items = cleanedItems;
      } else if (Array.isArray(parsed.list)) {
        nextValue.list = cleanedItems;
      } else if (Array.isArray(parsed.data)) {
        nextValue.data = cleanedItems;
      } else if (
        parsed.data &&
        Array.isArray(parsed.data.shops)
      ) {
    ...
    console.warn("ADMIN DASHBOARD STORAGE CLEAN ERROR:", e.message);
  }
}
...
function isHiddenDashboardShop(item = {}) {
  if (isHiddenDashboardName(getDashboardItemText(item))) {
    return true;
  }
  ...
  if (
    ...
    try {
      ...
      return HIDDEN_DASHBOARD_SHOP_NAMES.some(
        (hiddenName) =>
          rawText.includes(hiddenName)
      );
    } catch {
      return false;
    }
...
function getStoredShopsByCategory(category) {
  try {
    ...
    keys.forEach((key) => {
      ...
      const items = collectArrayFromStoredValue(value);

      const fallbackCategory = normalizeDashboardCategory(key);
    ...
    collected.forEach((item) => {
      const key = getShopIdentity(item);
    ...
    return sanitizeDashboardShops(
      Array.from(uniqueMap.values())
    );
  } catch (e) {
    console.warn("ADMIN LOCAL SHOP COUNT ERROR:", e.message);
...
async function fetchDashboard(category) {
  try {
    ...
    if (
      res.status === 401 ||
      data?.msg === "AUTH_TOKEN_REQUIRED" ||
      data?.message === "인증 토큰 없음" ||
      data?.msg === "INVALID_TOKEN"
    ) {
      return {
        authRequired: true,
      };
    ...
    console.error("ADMIN DASHBOARD FETCH ERROR:", e.message);

    const fallbackSnapshot =
      await fetchShopDashboardSnapshot(category);
...
async function fetchShopDashboardSnapshot(category) {
  ...
  try {
    list =
      normalizeShopSnapshotResponse(
        await shopApi.getList(categoryParams)
      );
  } catch (e) {
    console.error(
  ...
  toArray(list.shops).forEach((shop) => {
    if (
      shop &&
      typeof shop === "object" &&
      !isDeletedShop(shop) &&
      ...
      uniqueMap.set(getShopIdentity(shop), shop);
    }
  });

  const shops =
    sanitizeDashboardShops(
      Array.from(uniqueMap.values())
    );
...
function mergeShopSnapshotIntoDashboard(dashboardData, shopSnapshot) {
  ...
  return {
    ...
    summary: {
      ...
      totalShops: snapshotShops.length,
    },
    recent: {
      ...EMPTY_DASHBOARD_DATA.recent,
      ...(safeDashboard.recent || {}),
      shops: sanitizeRecentDashboardShops(snapshotShops),
    },
  };
}
...
function AdminDashboard() {
  ...
  const load = async () => {
    try {
      ...
      removeHiddenDashboardShopsFromStorage(dashboardCategory);

      const res = await fetchDashboard(dashboardCategory);
      ...
      const shopSnapshot =
        await fetchShopDashboardSnapshot(dashboardCategory);
  ...
  const safeRecent = {
    shops: sanitizeDashboardShops(recent?.shops),
    users: recent?.users || [],
  ...
  const safeSummary = {
    ...summary,
    totalShops: safeRecent.shops.length,
  };
        ...
        {activeTab === "dashboard" && (
          <>
            <div style={styles.grid}>
              <Card
                title="매장 수"
                value={safeSummary.totalShops}
              />

              <Card
                title="유저 수"
                value={safeSummary.totalUsers}
              />

              <Card
                title="예약 수"
                value={safeSummary.totalReservations}
              />

              <Card
                title="결제 수"
                value={safeSummary.totalPayments}
              />

              <Card
                title="총 매출"
                value={safeSummary.totalRevenue}
              />
            </div>
            ...
            <div style={styles.section}>
              ...
              <RecentShopList items={safeRecent.shops} />
            </div>

            <div style={styles.section}>
              <h3>최근 유저</h3>

              <List items={safeRecent.users} />
            </div>

            <div style={styles.section}>
              <h3>최근 예약</h3>

              <List items={safeRecent.reservations} />
            </div>
          </>
        )}
...
function RecentShopList({
  items,
}) {
  const visibleItems =
    sanitizeDashboardShops(items);
  ...
  return (
    <div style={styles.list}>
      {visibleItems.map((item, idx) => {
        ...
        if (isHiddenDashboardName(displayText)) {
          return null;
        }
