...
function isHiddenDashboardName(value) {
  const text =
    normalizeHiddenDashboardText(value);
...
function removeHiddenDashboardShopsFromStorage(category) {
  try {
    ...
    keys.forEach((key) => {
      ...
      const originalItems =
        collectArrayFromStoredValue(rawValue);
    ...
    console.warn(
      "ADMIN DASHBOARD STORAGE CLEAN ERROR:",
      e.message
    );
  }
}
...
function AdminDashboard() {
  ...
  const load = async () => {
    try {
      ...
      removeHiddenDashboardShopsFromStorage(
        dashboardCategory
      );
