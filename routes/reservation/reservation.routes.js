...
function safeRequire(modulePath) {
  try {
    ...
    console.warn("[reservation.routes] require fail:", modulePath, error.message);
    return null;
  }
}
...
function isDbBypassRoute(req) {
  const path = String(req.path || "");
  return path === "/ping" || path === "/system/status" || path === "/debug/final";
}

function safeAsync(fn) {
  return (req, res, next) => {
    if (!isDbReady() && !isDbBypassRoute(req)) {
      return fail(res, 503, { message: "DB_NOT_CONNECTED" });
    }
