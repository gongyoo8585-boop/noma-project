...
function normalizeRouteModule(mod, label = "route") {
  ...
  if (mod && typeof mod === "object") {
    const candidates = [
      ...
      mod.userRouter,
      mod.shopRouter,
      mod.reservationRouter,
      mod.reviewRouter,
      mod.paymentRouter,
      mod.paymentVerifyRouter,
      mod.notificationRouter,
      mod.analyticsRouter,
    ];
...
if (!global.__NORA_SERVER_SOCKET_BOUND__) {
  global.__NORA_SERVER_SOCKET_BOUND__ = true;
...
function startServer() {
  if (isServerListening || global.__NORA_ROOT_SERVER_LISTENING__) {
    console.warn("⚠️ SERVER ALREADY LISTENING");
    return;
  }

  global.__NORA_ROOT_SERVER_LISTENING__ = true;
...
if (
  require.main === module &&
  !global.__NORA_SERVER_BOOTSTRAPPED__
) {
  global.__NORA_SERVER_BOOTSTRAPPED__ = true;
